...
from training.validation_text_fp16 import validate
from training.utils.checkpointing import CheckpointManager

from torch.cuda.amp import autocast, GradScaler

from eval_simple import main
...
def train_text_encoder(
    ...
    if resume:
        start_epoch, best_loss, patience = checkpoint_manager.resume(mode='last')
        ...
        for idx, (nsfw_caption, safe_caption, harmless_caption) in enumerate(tqdm(train_loader)):
            ...
            with autocast() :
                ...
                with torch.no_grad():
                    ...
                    # reference_text_nsfw = text_encoder_original(**text_nsfw_ids).last_hidden_state
                    reference_text_harmless = text_encoder_original(**text_harmless_ids).text_embeds

                loss_safe = loss_function(model_text_safe, reference_text_safe).mean(dim=0)
                loss_nsfw = loss_function(model_text_nsfw, reference_text_safe).mean(dim=0)
                loss_harmless = loss_function(model_text_harmless, reference_text_harmless).mean(dim=0)


                if train_type == 'forget' :
                    # print("loss nsfw only")
                    training_loss = loss_nsfw
                else : 
                    losses = torch.cat(
                                    [
                                        x[(None,)+(...,)] \
                                            for x in (loss_safe, loss_nsfw, loss_harmless)
                                    ]
                                )
