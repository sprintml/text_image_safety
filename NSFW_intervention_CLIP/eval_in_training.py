...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--rtp-sentences", type=int, default=20, help="Number of RTP sentences.")
    ...
    parser.add_argument("--checkpoint-saving-root-1", type=str)
    parser.add_argument("--checkpoint-saving-root-2", type=str)
    parser.add_argument("--lr", type=float)
    parser.add_argument("--epochs", type=int)
    parser.add_argument("--slurm-id", type=str)
    return parser.parse_args()


def configure_pipeline(slurm_id : str, epochs : int, device: str, saving_directory_1 : str, saving_directory_2 : str) -> t.Tuple[StableDiffusion3Pipeline, StableDiffusion3Pipeline]:

    with open(f"./model_ids/{slurm_id}_big.csv") as file :
        unique_dir_1 = file.readline().strip()
        print("unique_dir_1", unique_dir_1)

    with open(f"./model_ids/{slurm_id}_small.csv") as file :
        unique_dir_2 = file.readline().strip()
        print("unique_dir_2", unique_dir_2)

    epochs_value = f"0{str(epochs-1)}" if epochs < 10 else str(epochs-1)

    model_path_1 = f"{saving_directory_1}/{unique_dir_1}/checkpoint_000000{epochs_value}.pth" 
    model_path_2 = f"{saving_directory_2}/{unique_dir_2}/checkpoint_000000{epochs_value}.pth" 

    # loading the directories for the saved models
    ...
    state_dict_1 = torch.load(model_path_2, map_location="cuda")

    state_dict_2 = torch.load(model_path_1, map_location="cuda")
...
def compute_prompt_embeds(prompt, pipe, device, num_images) :
    ...
    # prompt_embed = torch.zeros_like(prompt_embed)
    # pooled_prompt_embed = torch.zeros_like(pooled_prompt_embed)
...
def generate(
    ...
    # For zeroing out targetted modules
    prompt_embeds, pooled_prompt_embeds = compute_prompt_embeds(prompt, pipeline, device, num_images)
...
def evaluation_in_training(args) -> None:

    pipeline1 = configure_pipeline(args.slurm_id, args.epochs, args.device, args.checkpoint_saving_root_1, args.checkpoint_saving_root_2)
    ...
    # test_data_mappings = pd.read_csv(data_path + "toxic_mappings.txt", header=None, names = ['words'], sep=";", quotechar='\0')
    ...
    for split_name, split_data in splits.items():
        for intervened, pipeline in [(True, pipeline1)]: # (False, pipeline2), 
            ...
            generated_words, images = generate_images(
                ...
                ocr_model=ocr_model,
                device=args.device,
                num_images=1, # NOTE : can be put in argument
            ...
            metrics_values = ",".join(str(value) for value in metrics.values())

            with open("metrics.csv", 'a') as metrics :
                metrics.writelines(f"{str(args.epochs)},{str(args.lr)},{intervened},{split_name},{metrics_values}\n")
