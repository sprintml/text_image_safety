...
class StableDiffusionXLPipeline(
    ...
    def __call__(
        ...
        start_denoising_from_timestep: int = 0,
        **kwargs,
    ):
        r"""
        ...
        if self.do_classifier_free_guidance:
            ...
            if prompt_B is not None:
                prompt_embeds_B = torch.cat([negative_prompt_embeds_B, prompt_embeds_B], dim=0)
                add_text_embeds_B = torch.cat([negative_pooled_prompt_embeds_B, pooled_prompt_embeds_B], dim=0)
                add_time_ids_B = torch.cat([negative_add_time_ids, add_time_ids], dim=0)

        prompt_embeds = prompt_embeds.to(device)
        add_text_embeds = add_text_embeds.to(device)
        add_time_ids = add_time_ids.to(device).repeat(batch_size * num_images_per_prompt, 1)
        if prompt_B is not None:
            prompt_embeds_B = prompt_embeds_B.to(device)
            add_text_embeds_B = add_text_embeds_B.to(device)
            add_time_ids_B = add_time_ids_B.to(device).repeat(batch_size * num_images_per_prompt, 1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt:
                    continue
                if i < start_denoising_from_timestep:
                    continue
                ...
                noise_pred = self.unet(
                    ...
                    encoder_hidden_states_B=prompt_embeds_B,
                )[0]
