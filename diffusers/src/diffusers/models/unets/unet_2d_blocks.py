...
from ...utils.int_wrapper import IntWrapper
...
class UNetMidBlock2DCrossAttn(nn.Module):
    ...
    def forward(
        ...
        curr_transformer_idx: Optional[IntWrapper] = None,
        ...
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = attn(
                    ...
                    encoder_attention_mask=encoder_attention_mask,
                    return_dict=False,
                    curr_transformer_idx=curr_transformer_idx,
...
class UNetMidBlock2DSimpleCrossAttn(nn.Module):
    ...
    def forward(
        ...
        batch_num: Optional[int] = None,
        is_last_timestep: Optional[bool] = False,
        timestep_start_patching: Optional[int] = None,
        encoder_hidden_states_B: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        for attn, resnet in zip(self.attentions, self.resnets[1:]):
            ...
            if isinstance(attn_idx_to_patch, int):
                patch_attn = attn_idx_to_patch == curr_transformer_idx.value
                attn_heads_to_patch = attn_heads_idx_to_patch if patch_attn else None
            elif isinstance(attn_idx_to_patch, list):
                patch_attn = curr_transformer_idx.value in attn_idx_to_patch
                # attn_heads_idx_to_patch should be a dict or None
                if isinstance(attn_heads_idx_to_patch, dict):
                    attn_heads_to_patch = attn_heads_idx_to_patch[curr_transformer_idx.value] if patch_attn else None
                else:
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            if self.training and self.gradient_checkpointing:
                ...
                if isinstance(attn_idx_to_patch, int):
                    ...
                    if isinstance(attn_heads_idx_to_patch, dict):
                        attn_heads_to_patch = (
                            attn_heads_idx_to_patch[curr_transformer_idx.value] if patch_attn else None
                        )
                    else:
                        attn_heads_to_patch = None
                else:
    ...
    def forward(
        ...
        for idx, (resnet, attn) in enumerate(zip(self.resnets, self.attentions)):
            ...
            if self.training and self.gradient_checkpointing:
                ...
                # hidden_states = resnet(hidden_states, temb_B if inject_B else temb) TODO
                hidden_states = resnet(hidden_states, temb)
                hidden_states = attn(
                    hidden_states,
                    encoder_hidden_states=encoder_hidden_states,
